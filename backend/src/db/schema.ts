/**
 * XAMOTO — Schéma de base de données (§32).
 *
 * Le même schéma logique existe en deux dialectes :
 *   – SQLite embarqué (local-first, développement, démonstration hors ligne),
 *   – PostgreSQL / Supabase (production, voir database/migrations/postgres).
 *
 * La liste des tables reprend intégralement celle du cahier des charges :
 * users, vehicles, vehicle_specs, obd_devices, obd_sessions, obd_data,
 * dtc_codes, diagnostic_sessions, diagnostic_findings, diagnostic_tests,
 * diagnostic_test_results, repairs, maintenance, vehicle_events,
 * vehicle_documents, vehicle_passport, parts, part_compatibility, garages,
 * garage_services, quotes, inspections, ai_conversations, ai_messages,
 * knowledge_documents, knowledge_sources, alerts, notifications, audit_logs.
 *
 * Tables additionnelles justifiées par le produit :
 *   organizations (flottes/garages), vehicle_shares (§36 permissions),
 *   reports (§25), sync_operations (§29), sessions (authentification).
 */
...
/** Liste des tables, utilisée par les tests et l'écran « administration ». */
export const TABLES = [
  'organizations', 'users', 'sessions',
  'vehicles', 'vehicle_shares', 'vehicle_specs',
  'obd_devices', 'obd_sessions', 'obd_data', 'dtc_events', 'dtc_codes',
  'diagnostic_sessions', 'diagnostic_findings', 'hypotheses', 'diagnostic_tests', 'diagnostic_test_results',
  'repairs', 'repairs_verification', 'maintenance', 'vehicle_events', 'vehicle_documents', 'vehicle_passport', 'inspections', 'reports',
  'garages', 'garage_services', 'quotes', 'parts', 'part_compatibility',
  'ai_conversations', 'ai_messages', 'knowledge_sources', 'knowledge_documents',
  'alerts', 'notifications', 'audit_logs', 'sync_operations',
] as const;
